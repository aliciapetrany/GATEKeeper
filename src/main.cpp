...
// TODO - 
// make flag to make vcf outputs optional
// time metadata file
bwt_t *Refbwt;
string cmd_line;
bwaidx_t *RefIdx;
...
bool bDebugMode, bDUPmode, bSensitive, bVCF, bShowPlot, bAllowDuplication, OneOnOneMode, Verbose;  // Alicia, added verboisty
...
void ShowProgramUsage(const char* program)
{
	...
	fprintf(stderr, "         -v             Verbose output [false]");		
	fprintf(stderr, "\n");
}
...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		for (i = 1; i < argc; i++)
		{
			...
			else if (parameter == "-v") Verbose = true; //Alicia - added verbose parameter
	...
	if (Verbose) fprintf(stderr, "Step1. Load the two genome sequences...\n"); // Alicia, verbosity switch
	...
	if (Verbose) fprintf(stderr, "	Running BWT... \n");
	if (IndexFileName != NULL && CheckBWAIndexFiles(IndexFileName)){
		RefIdx = bwa_idx_load(IndexFileName);
	}
	...
	else
	{
		...
		GenomeComparison();
		if (bVCF){ 
			if(Verbose) fprintf(stderr, "\nGATEkeeper identifies %d SNVs, %d insertions, and %d deletions [%s].\n\n", iSNV, iInsertion, iDeletion, vcfFileName);
			OutputSequenceVariants();   //Alicia Verbosity switch
		}
