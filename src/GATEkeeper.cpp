...
//To find my changes, just control F "Verbose" - Alicia
...
void *IdentifyLocalMEM(void *arg)
{
	...
	while (true)
	{
		...
		if (Verbose) fprintf(stderr, "\r\t\tSeed exploration: %d / %d (%d%%)...", stop, QryChrLength, (int)(100 * ((1.0*stop / QryChrLength))));
		pthread_mutex_unlock(&Lock);
...
void GenomeComparison()
{
	...
	if (Verbose) fprintf(stderr, "Step2. Sequence analysis for all query chromosomes\n");
	for (QueryChrIdx = 0; QueryChrIdx != iQueryChrNum; QueryChrIdx++)
	{
		...
		if (Verbose) fprintf(stderr, "\tProcess query chromosome: %s...\n", QueryChrVec[QueryChrIdx].name.c_str());
		...
		if (Verbose) fprintf(stderr, "\t\tSeed clustering and chaining..."); //iThreadNum = 1;
		for (i = 0; i < iThreadNum; i++) pthread_create(&ThreadArr[i], NULL, GenerateAlignmentBlocks, &vec[i]);
		for (i = 0; i < iThreadNum; i++) pthread_join(ThreadArr[i], NULL); 
		if (Verbose) fprintf(stderr, "\n");
		if (Verbose) fprintf(stderr, "\t\tFix overlapping seeds and close gaps between gaps...");
		...
		if (Verbose) fprintf(stderr, "\t\tGenreate sequence alignment..."); //iThreadNum = 1;
		...
		RemoveBadAlnBlocks();
		if (n == 0) continue;
		if (Verbose) fprintf(stderr, "\t\tProduce %d local alignments (length = %lld), ANI=%.2f%%\n", n, (long long)aln_len, (100*(1.0*aln_score/aln_len)));
		if (OutputFormat == 1){
			if (Verbose) fprintf(stderr, "\t\tOutput alignments for query sequence (%s)\n", mafFileName); 
			OutputMAF();
		}
		if (OutputFormat == 2){
			if (Verbose) fprintf(stderr, "\t\tOutput alignments for query sequence (%s)\n", alnFileName);
		...
		if (bVCF){
			if (Verbose) fprintf(stderr, "\t\tIdentify sequence variants for query sequence...\n");
			VariantIdentification();
		}
		if (bShowPlot && GnuPlotPath != NULL){
			if (Verbose) fprintf(stderr, "\t\tGenerate dotplot for query sequence (%s-%s.ps)\n", OutputPrefix, QueryChrVec[QueryChrIdx].name.c_str());
			OutputDotplot();
		}
		if (Verbose) fprintf(stderr, "\n");
	...
	if (LocalAlignmentNum > 0 && Verbose) fprintf(stderr, "\tAlignment#=%d (total alignment length=%lld) ANI=%.2f%%, unique alignment#=%d\n", (int)LocalAlignmentNum, (long long)TotalAlignmentLength, (100 * (1.0*TotalAlignmentMatches / TotalAlignmentLength)), int(LocalAlignmentNum - DupAlnNum));
	if (Verbose) fprintf(stderr, "\tIt took %lld seconds for genome sequence alignment.\n", (long long)(time(NULL) - StartProcessTime));
